...
"!GPTMOSS_PYTHON!" -B -c "import sys; raise SystemExit(0 if sys.version_info >= (3, 10) else 1)"
if !errorlevel! neq 0 (
...
if /i "!GPTMOSS_RUNTIME_KIND!"=="venv" (
    ...
    "!GPTMOSS_PYTHON!" -B -c "import venv" >nul 2>&1
    if !errorlevel! neq 0 (
    ...
    "!GPTMOSS_PYTHON!" -B -m venv "%~dp0venv"
    if !errorlevel! neq 0 (
...
if exist "%~dp0wheelhouse\*.whl" (
    ...
    "%~dp0venv\Scripts\python.exe" -B -m pip install --no-index --find-links "%~dp0wheelhouse" -r "%~dp0requirements.txt"
) else (
    "%~dp0venv\Scripts\python.exe" -B -m pip install -r "%~dp0requirements.txt"
)
if !errorlevel! neq 0 (
