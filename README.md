...
Les tâches de rédaction professionnelle activent le profil `professional-local`. Le moteur impose alors son propre plancher de qualité, inventorie les pièces jointes réelles et refuse les textes trop courts, dupliqués, non sourcés ou contenant des placeholders. Lorsqu'un grand document existant échoue uniquement sur sa longueur, une section vide ou ses références locales manquantes, le tour de réparation suivant est borné au seul appel `filesystem.append` attendu ; si l'artefact n'existe pas encore, seul `filesystem.write` est exposé. Les défauts qui doivent retirer du contenu (doublons, référence invalide, lien externe, placeholder) imposent plutôt un premier `write` borné et propre, puis des `append` successifs. Cette contrainte transitoire empêche les boucles de relecture sans mutation et disparaît dès l'appel exécuté. Un artefact obligatoire de l'étape active ne peut jamais être supprimé, directement ou au moyen d'une commande shell, ni remplacé par un contenu vide : il doit être réparé en place. Après réussite de l'assurance finale, GPTMOSS produit sous `.gptmoss/deliveries/<execution-id>/` un DOCX mis en forme, le rapport d'assurance, un manifeste SHA-256 et une archive ZIP téléchargeable depuis l'interface. Ce paquet n'est jamais créé avant le passage des contrôles.
