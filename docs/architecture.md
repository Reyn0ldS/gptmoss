...
| Contexte et mémoire | `core/context.py`, `memory/json_store.py`, `capabilities/memory.py` | Contexte borné, digest des `documents.read` durables après compactage, et mémoire gouvernée par projet, validation, provenance, TTL, déduplication et supersession. |
...
| Fournisseur | `providers/qwen.py`, `providers/qwen_support.py` | Adaptateur OpenAI-compatible, TLS sûr (échec certificat = configuration, pas `waiting_provider`), vision, délai de lecture `llm_timeout_seconds`, préflight jetons/outils, réserve de sortie, apprentissage de la fenêtre réelle, compactage et reprise. |
