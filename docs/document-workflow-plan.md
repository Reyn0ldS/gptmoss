...
Le second essai réel a révélé un faux positif de handoff : après trois lectures correctes, un spécialiste a inventé l'identifiant du PPTX. Il a bien ré-inventorié le corpus, mais la récupération d'inactivité a ensuite généré hors contexte `analysis/corpus-inventory.md`; la seule présence du fichier avait suffi à lancer l'étape dépendante. Le fichier contenait des références inventées, des ellipses de remplacement et une balise `</think>`. La correction générale interdit désormais le secours génératif des livrables textuels lorsqu'un corpus est joint, résout de façon sûre les noms et empreintes parmi les seules pièces jointes, applique les validateurs à chaque handoff, propage les politiques aux sous-exécutions, refuse les placeholders et balises de raisonnement, et impose des politiques déterministes aux 14 artefacts du plan réel. Le mauvais fichier réel est maintenant refusé par quatre gates indépendants. La sélection automatique des skills exige également plus qu'un token incident comme `vision.pptx`. Validation : 61 tests ciblés et 197 tests complets réussis.
...
Le spécialiste borné n'a plus créé de sous-agent et les protections d'ownership ont refusé ses tentatives hors chemin. Il persistait toutefois à viser les rapports qualité car le mapping lexical lui avait transmis 16 exigences, dont les fichiers des étapes ultérieures. Le fallback documentaire possède maintenant un mapping déterministe par nature de livrable : l'inventaire réel ne reçoit plus que six règles cohérentes (sources locales, inventaire, lecture, numéros réels, absence de placeholders, relecture). Les rapports sont produits après réparation par un rédacteur de preuves, puis un QA distinct écrit `analysis/final-delivery-audit.md` et valide les 27 exigences avant le coordinateur. Le prompt rappelle explicitement que les exigences héritées n'élargissent jamais `owned_paths`. Rejeu exact : 13 étapes, quatre rapports possédés par l'étape 10, audit indépendant à l'étape 11, toutes les exigences validées ; 59 tests ciblés et 202 tests complets réussis.

La reprise du jalon a ensuite isolé une ambiguïté de coordonnées dans les outils documentaires : `documents.read start_block` attend un offset de bloc normalisé à base zéro, tandis que les références du livrable utilisent des bornes à base un et les PPTX citent des numéros de diapositives. L'inventaire expose désormais séparément `normalized_block_offsets` et `citation_bounds`, et le prompt spécialiste interdit de déduire une diapositive depuis l'ordre ou le nombre des blocs. Le validateur rapporte aussi la plage invalide et la plage attendue sans masquer une borne inversée. Le nombre de diapositives vient des métadonnées du parseur afin de conserver une diapositive sans bloc textuel. Validation : 53 tests documentaires et d'orchestration ciblés, puis 206 tests complets réussis.
